...
class RBM:
    ...
    def update(self,v_d,h_d,v_h,h_h,mh_h):
        ...
        if self.regu: # Regularización L2
            ...
            self.W += self.lr*(h_d.mm(v_d.t())-mh_h.mm(v_h.t()))/self.S_batch + torch.randn(self.Nh, self.Nv)*np.sqrt(2*self.lr*self.T)
            
        self.b_v += self.lr*torch.mean((v_d-v_h),1) + torch.randn(self.Nv)*np.sqrt(2*self.lr*self.T)
        self.b_h += self.lr*torch.mean((h_d-mh_h),1) + torch.randn(self.Nh)*np.sqrt(2*self.lr*self.T)

    def Vsampling(self,H):
        
        mv = torch.sigmoid((self.W.t().mm(H).t()+self.b_v).t())
        v = torch.bernoulli(mv)
        return v,mv

    def Hsampling(self,V):
        
        mh = torch.sigmoid((self.W.mm(V).t()+self.b_h).t())
    ...
    def train(self,D): #introduzco dataset
        ...
        for t in range(emax): # bucle epochs
            ...
            for k in range(NB): #bucle minibatches
                ...
                self.update(v_d,h_d,v_h,h_h,mh_h) #actualizo pesos
